...
<?php
  ...
  if (!isset($_SESSION['username'])) {
    ...
    $result = $mysqli->query("SELECT * FROM pizza") or die($mysqli->error);
    ...
    while ($data = $result->fetch_assoc()) { 
      ...
      echo "<img src='{$imagePath}' alt='Pizza Image' onerror=\"this.onerror=null;this.src='https://via.placeholder.com/300x300?text=No+Image';\"/>";
      echo "<div>{$data['title']} - ₹{$data['price']}</div>";
      echo "<button class='add-to-cart' 
              data-id='{$data['id']}' 
              data-source-table='pizza' 
              data-title='{$data['title']}' 
              data-price='{$data['price']}' 
              data-image='{$imagePath}'>Add to Cart</button>";
      echo "<button class='remove-from-cart' 
              data-id='{$data['id']}' 
              data-source-table='pizza'>Remove</button>";
      echo "</div>";
    }
